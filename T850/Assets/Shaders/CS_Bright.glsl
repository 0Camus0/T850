...
layout(binding = 2) uniform sampler2D AdaptedLuminanceTexture;
layout(rgba8, binding = 5) writeonly uniform image2D OutputTexture;
...
void main()
{
    ...
    color = exp2(log2(max(linearExposure, 0.0001)) + OutputSizeAndParams0.w - OutputSizeAndParams0.z) * color;
    ...
    imageStore(OutputTexture, ivec2(dispatchId), vec4(toneMappedLuminance * (color / pixelLuminance), 1.0));
}