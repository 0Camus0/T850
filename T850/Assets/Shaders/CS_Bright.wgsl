...
@group(0) @binding(2) var adaptedLuminanceTexture: texture_2d<f32>;
@group(0) @binding(3) var hdrSampler: sampler;
@group(0) @binding(4) var luminanceSampler: sampler;
...
fn CS(@builtin(global_invocation_id) dispatchId: vec3<u32>) {
    ...
    var color = textureSampleLevel(hdrTexture, hdrSampler, uv, 0.0).rgb;
    ...
    let threshold = constants.outputSizeAndParams0.z;
    let exposureComp = constants.outputSizeAndParams0.w;
    color *= exp2(log2(max(linearExposure, 0.0001)) + exposureComp - threshold);
    ...
    textureStore(outputTexture, dispatchId.xy, vec4<f32>(toneMappedLuminance * (color / pixelLuminance), 1.0));
}