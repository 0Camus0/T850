...
namespace t800 {
  ...
  void Spline::ReCalculateSegmentsLength()
  {
    if (m_points.size() < 4) {
    ...
    for (std::size_t i = 0; i < (m_points.size()- 3); i++)
    {
      ...
      if (m_points[i].m_LookAtCenter) {
        float		_Yaw;
        float		_Pitch;
        XVECTOR3 Look = m_points[i+1] - m_points[i];
        Look.Normalize();
        _Pitch = asin(-Look.y);
        _Yaw = atan2f(Look.x, Look.z);
        m_points[i].m_rotation = XVECTOR3(_Pitch, _Yaw, 0.0f);
      }
    }
  }
  SplinePoint Spline::GetPoint(float t)
  {
    int p0, p1, p2, p3;
    SplinePoint ret;
    if (!m_looped)
    ...
    /*ROTATION*/
    ret.m_rotation = m_points[p1].m_rotation + ((m_points[p2].m_rotation - m_points[p1].m_rotation)*t);

    return ret;
  }
  XVECTOR3 Spline::GetGradient(float t)
  {
    int p0, p1, p2, p3;
    SplinePoint ret;
    if (!m_looped)
  ...
  float Spline::GetNormalizedOffset(float p)
  {
   int i = 0;
