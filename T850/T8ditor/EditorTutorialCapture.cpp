...
void EditorApp::PrepareTutorialCapture() {
  ...
  if (m_tutorialStep == "wireframe-depth-on" || m_tutorialStep == "wireframe-depth-off" ||
      ...
      m_tutorialStep == "placement-depth-on" || m_tutorialStep == "placement-depth-off") {
    const bool placementGrid = m_tutorialStep.starts_with("placement-depth-");
    ...
    m_panels.showSelectionWireframe = !placementGrid && m_tutorialStep.ends_with("-on");
    ...
    if (placementGrid) m_showPlacementGrid = m_tutorialStep.ends_with("-on");
    world.selectedIdx = 0;
    world.selectionType = 0;
    ...
    if (placementGrid) m_camera.SetOrbitState(0.7f, 0.65f, 75.0f);
