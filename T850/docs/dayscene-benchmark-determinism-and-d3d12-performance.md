...
DayScene benchmark mode has two modes. `--benchmarkSeconds` is the FPS/performance path: it runs unthrottled for a fixed real duration and advances the scripted scene from benchmark wall-clock elapsed time. `--benchmarkFrames` is the deterministic frame-validation path.

- Fixed simulation dt: `1.0 / 60.0`.
- Warmup frames: `30`.
- Measured duration: launchers request `90` wall-clock seconds for benchmark runs; direct CLI without an override still uses the 10 second debug default (`600` frames).
- `--benchmarkSeconds <seconds>` overrides measured duration. The benchmark runs unthrottled for that real duration and records however many frames the device can render while the camera follows the same elapsed-time timeline.
- `--benchmarkFrames <N>` overrides measured frames.
- `--benchmarkFixedDt <seconds>` overrides fixed simulation dt.

Measured frame times are recorded from actual rendered frame duration in `Application`, not from simulation dt.
...
for each API/resolution/mode:
  ...
  run until benchmarkSeconds elapsed, or measuredFrames in validation mode
  capture final frame at the end of the scripted timeline
...
- `--benchmarkSeconds <seconds>`: run the scripted benchmark unthrottled for a fixed wall-clock duration.
