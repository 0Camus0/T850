...
The filesystem `ShaderDiskCache` and the D3D12 persistence layers cover different artifacts. `ShaderDiskCache` restores compiled stage bytecode and reflection. `D3D12ShaderCacheSession` restores individual graphics and compute cached PSO blobs after root-signature and descriptor construction. `D3D12PipelineLibrary` restores named PSOs from one driver-native library stored as a value in that session. Session keys include stable program/pipeline state and shader bytecode hashes; the D3D12 runtime additionally versions storage by driver, and the engine increments its session version when the schema changes. The serialized PipelineLibrary input remains owned by the wrapper for the library's lifetime, as required by `CreatePipelineLibrary`.
...
The schema-2 five-process x64 matrix also measures PipelineLibrary after all three native modes are warm. The median ten-pipeline total was 14.273 ms with 50/50 library hits and a 24,952-byte serialized library, compared with 16.243 ms for 50/50 per-PSO session hits and 14.862 ms with both application caches disabled. PipelineLibrary was 4.0% below the driver-only control at this boundary. The 12.1% difference from the session path should not be read as a stable ordering: these sub-2 ms deltas are sensitive to process noise, and an earlier matrix measured a small session benefit over the driver-only path. This is evidence that grouping Dawn's D3D12 PSOs into an application-managed PipelineLibrary may reduce warm startup work; it is not a measured Dawn improvement. A direct Dawn experiment would still need persistent BlobCache callbacks and an A/B PipelineLibrary implementation on the same devices.
