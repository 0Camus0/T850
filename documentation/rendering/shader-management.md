...
- D3D12 owns typed `D3D12Pipeline` graphics objects in the driver, keyed by program plus fixed render state and attachment formats. Cached graphics and compute PSO blobs persist through a driver-versioned `ID3D12ShaderCacheSession`; stale blobs are rejected and recreated without failing the draw. Set `T850_D3D12_SHADER_CACHE_SESSION=off` for a session-disabled control or `reset` to delete and repopulate the application session. An experimental `ID3D12PipelineLibrary` layer groups named graphics and compute PSOs into one serialized library. It is opt-in with `T850_D3D12_PIPELINE_LIBRARY=on`; `reset` creates and repopulates a new library. Unsupported drivers fall back to the per-PSO session path. Invalid serialized libraries are rejected and repopulated from an empty library. Cross-process library persistence requires `T850_D3D12_SHADER_CACHE_SESSION` to remain enabled. These controls do not disable the Windows/driver-managed D3DSCache.
...
- WebGPU owns render pipelines in `WebGPUDriverState`, keyed by program, resource layout, vertex layout, targets, topology, blend, depth, and cull state. Native Dawn/D3D12 receives a `DawnCacheDeviceDescriptor` with thread-safe disk callbacks under `Shaders/.t8shadercache/dawn-native`; Dawn's opaque keys and payload validation remain authoritative. Persistence defaults on. Set `T850_WEBGPU_DAWN_CACHE=off` for a callback-disabled control or `reset` to delete and repopulate Dawn's BlobCache. The Windows/driver-managed D3DSCache may remain active in either mode. Browser WebGPU keeps browser-managed caching and does not use these native callbacks.
...
Windows also maintains a per-user D3D disk cache under `%LOCALAPPDATA%/D3DSCache`. On the ARM64 Adreno host, benchmark-correlated SQLite databases were stored under `C:/Users/erickdaniele/AppData/Local/D3DSCache`; their filenames included the Qualcomm device identity `VEN_4D4F4351&DEV_36334330`. The main 48-entry database was active from the historical native run through the current Dawn/native matrix. Therefore `T850_D3D12_SHADER_CACHE_SESSION=off`, `T850_D3D12_PIPELINE_LIBRARY=off`, and `T850_WEBGPU_DAWN_CACHE=off` disable only T850-managed persistence layers. They are driver-warm controls, not all-layer uncached controls.

A separate ARM64 driver-cold control kept T850's DXIL/reflection artifacts warm, disabled ShaderCacheSession and PipelineLibrary, and deleted DayScene's application-specific D3DSCache directory before each of five measured processes. Every run began with the directory absent and recreated two Qualcomm-qualified SQLite databases. The ten-pipeline median was 64.541 ms (64.488-70.422 ms), versus 8.389 ms when T850 application caches were off but D3DSCache remained warm. This is the valid reference for the cost of removing the Windows/driver cache layer; the 8.389 ms value must not be described as uncached.
...
`scripts/CapturePipelineReadyMatrix.ps1` measures the ten controlled compute permutations from manifest validation and source loading through successful usable pipeline creation. Engine logging is restricted to `error`; the structured timing line is emitted to stdout only after the measured interval. Five alternating T850-artifact-cold runs clear the selected T850 artifact cache before every process; they do not clear Windows D3DSCache. Warm runs start new processes after all flow caches are primed. D3D12 warm runs compare the same DXIL/reflection artifacts with PipelineLibrary, per-PSO application session, and both T850 application caches disabled. Native WebGPU warm runs compare the same T850 WGSL/SPIR-V artifacts with Dawn's persistent BlobCache callbacks on and off. Windows/driver caching may remain active in every cell. Schema 3 also retains the four priming-process records so cross-process stores and subsequent hits are auditable.
...
| D3D12 T850 caches off, D3DSCache warm | 15.299 ms | 14.863-16.359 ms | 865.3 ms | confirmed Windows/driver cache remains |
| Dawn WGSL BlobCache | 18.341 ms | 17.803-18.623 ms | 1811.0 ms | 1250/1250 callback hits |
| Dawn WGSL callbacks off, D3DSCache warm | 95.864 ms | 94.513-99.161 ms | 2478.6 ms | no persistent Dawn callbacks; Windows/driver cache remains |
| Dawn SPIR-V BlobCache | 20.155 ms | 19.948-23.201 ms | 2016.9 ms | 1250/1250 callback hits |
| Dawn SPIR-V callbacks off, D3DSCache warm | 114.787 ms | 111.505-131.841 ms | 2769.3 ms | no persistent Dawn callbacks; Windows/driver cache remains |
