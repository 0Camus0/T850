...
The command creates a minimal application without scene assets, compiles `Shaders/CS_Arithmetic.hlsl`, dispatches two 64-thread workgroups over 96 integers, reads the output back, and exits. D3D12 remains the default; add `--api d3d11` or `--api vulkan` for the other native implementations. Require `[ComputeSelfTest] PASS` and exit code 0 on all three. OpenGL must reject the workload with `API 'gl' does not support compute shaders` and a nonzero exit. This proves shared constants, a structured UAV/storage buffer, dispatch, synchronization, completion, and readback; it does not prove texture compute outside D3D12.
...
DayScene's God Rays calculation uses compute on D3D11, D3D12, and Vulkan when `--postProcessMode auto` or `compute` is selected. At 1280x720, require `Pass 'God Rays' dispatched compute 160 x 90 x 1`. D3D12 PIX must show `CS_GodRays.hlsl Compute PSO`, two sampled depth textures, two samplers, and the `GodRaysCalc` storage UAV. `--postProcessMode raster` must suppress that compute pipeline on each native compute API. OpenGL must use graphics fallback in every mode.

Post-processing uses `--postProcessMode auto|compute|raster`. In `auto`, DayScene selects CS for the preferred God Rays calculation and PS for the unbenchmarked God Rays blur H/V, Bright, and HDR Composition alternatives. `compute` forces every declared alternative for A/B testing, and `raster` suppresses all compute pipelines. At 1280x720, forced compute requires 160x90 God Rays/blur/HDR dispatches and a 64x64 Bright dispatch. Shadow and bloom blur remain raster. On OpenGL, forcing compute must log graphics fallback for every declared pass and must create or dispatch no compute pipeline. Matched D3D11, D3D12, and Vulkan replay comparisons keep every intermediate target within tolerance 2; the final backbuffer can contain a sparse cross-stage quantization delta up to 4 and must remain below 0.05% changed pixels.

All maintained render graphs must create and dispatch `CS_Bright` and `CS_HDRComposite` when compute is forced. Validate DayScene scenes 0-3 and 5-6 directly; SceneTemplate scene 4 requires an explicit `--sceneFile`. Also run T8ditor separately because it has its own CLI/parser and graph. D3D11 and Vulkan shared-graph plus Minecraft smoke tests must dispatch both kernels; matching OpenGL tests must log two or more graphics fallbacks, zero compute dispatches, and complete a nonuniform frame dump.
...
Run from `bin/x64/Debug` with `--logLevel info` and a unique log per API/mode. Test `compute` and `raster` on D3D11, D3D12, and Vulkan. For OpenGL, request `compute` and require the logged graphics fallback; OpenGL has no compute implementation.
...
Expected compute log evidence on each native compute API is pipeline creation followed by `Pass 'God Rays' dispatched compute 320 x 180 x 1 for 2560x1440 output`. `--postProcessMode raster` must report the graphics implementation and must not mention `CS_GodRays` or `dispatched compute`. OpenGL compute mode must report `using graphics fallback on API=gl`.
